...
/// <summary>
/// Folder tags under `audio`. A second tagging layer addressed by path, keyed by
/// the path in the body rather than by a parent id. There is no delete: the
/// collection offers none.
/// </summary>
public static class AudioFolderCommands
{
    private static readonly NLog.Logger _logger = NLog.LogManager.GetCurrentClassLogger();

    public static Command Create()
    {
        var command = new Command("folders", "Audio folders and their tags");
    ...
    {
        var command = new Command("list", "List tagged audio folders");
        ...
        command.AddExamples("grimoire-cli audio folders list");
        command.AddResponseExample<Generated.Models.AudioFoldersResponse>();
    ...
    private static Command CreateSetCommand()
    {
        ...
        var command = new Command("set", "Replace one audio folder's tags")
        {
        ...
        command.AddHelpSection("Notes", HelpSectionPosition.Top,
            ...
            "permanent.",
            "",
            "A tag reaches every audio track at or below the path in tags items and",
            "search. audio get matches folder_path exactly, so folder_tags on a",
            "track in a subfolder of the tagged path reads empty.");
        command.AddExamples(
            "echo '{\"path\":\"Ambience\",\"tags\":[\"ambience\"]}' | grimoire-cli audio folders set --stdin");
        command.AddRequestShape<Generated.Models.FolderTagsUpdate>();
        command.AddResponseExample<Generated.Models.Backend__routers__audio___schemas__FolderTagsOut>();
        command.SetAction(async (parseResult, cancellationToken) =>
        {
            string body;
            try
    ...
    private static Command CreateBatchSetCommand()
    {
        var inputOption = new Option<string?>("--input") { Description = "Read the body from this file" };
        var stdinOption = new Option<bool>("--stdin") { Description = "Read the body from stdin" };
        var command = new Command("batch-set", "Set tags on many audio folders in one transaction")
        {
            inputOption, stdinOption
        };
        command.AddRoleRequired("gm or admin");
        ...
        command.AddExamples("grimoire-cli audio folders batch-set --input folders.json");
