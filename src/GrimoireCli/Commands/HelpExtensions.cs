...
/// <summary>
/// Adds custom sections (Notes, Examples, request and response shapes) to
/// help output. Top-positioned sections render before the default layout;
/// Bottom-positioned sections render after Options in registration order.
/// </summary>
public static class HelpExtensions
{
    ...
    /// <summary>
    /// Registers a shape section, hidden behind --help-full. Callers pass content
    /// derived from generated samples rather than hand-written text — see
    ...
    /// and kept in sync by <c>ResponseExamplesDriftTest</c>) — Grimoire's OpenAPI
    /// spec types every response as an empty schema, so it's useless as a shape
    /// source for these — and <see cref="AddRequestShape{T}"/>, whose samples come
    /// from <c>RequestExamples.g.cs</c> instead.
    /// </summary>
    ...
    /// <summary>
    /// Registers the shape of the request body, hidden behind --help-full,
    /// rendering directly before the response shape when a command has one.
    /// The sample comes from <c>RequestExamples.g.cs</c>
