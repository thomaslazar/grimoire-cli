...
public static class HelpExtensions
{
    ...
    /// <see cref="AddResponseExample{T}"/>, <see cref="AddResponseExampleArray{T}"/>
    /// and <see cref="AddRequestShape{T}"/>, whose samples all come from
    /// <c>JsonExamples.g.cs</c> (generated by <c>tools/GenerateJsonExamples</c>
    /// from the generated Kiota models' own fields and property types, and kept
    /// in sync by <c>JsonExamplesDriftTest</c>) — except <c>SavedFile</c>, the
    /// CLI's own plain-POCO `--output` receipt, whose sample is a literal in the
    /// generator rather than walked from a model.
    /// </summary>
    ...
    public static void AddResponseExample<T>(this Command command)
        => command.AddShapeSection("Response shape", JsonExamples.For(typeof(T)).Split('\n'));
    ...
    {
        var element = JsonExamples.For(typeof(T));
    ...
    /// The sample comes from <c>JsonExamples.g.cs</c>, so its field names are
    /// exactly the set <see cref="JsonBodyInput.Validate"/> accepts and a field
    /// added to the API appears as soon as the client is regenerated.
    /// </summary>
