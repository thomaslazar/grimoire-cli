...
public static class FilesCommand
{
    ...
    private static Command CreateDeleteCommand()
    {
        ...
        command.AddHelpSection("Notes", HelpSectionPosition.Top,
            ...
            "re-adds whatever is still on disk — as new entries, so tags, favorites,",
            "bookmarks and progress do not come back. Works on a read-only library.",
            "A soft delete that empties a system folder also drops its system row,",
            "unless the system carries a custom name, description or cover, or a",
            "campaign references it.",
            "",
            "--delete-files also deletes the files, and cannot be undone.",
            "",
