...
- **No confirm-gated command, and this is now settled.** abs-cli exempts `libraries delete` from thin pass-through with a type-the-name prompt. `library cleanup-missing` was the command that decided it and takes neither a prompt nor a `--yes`: the callers here are agents, so a prompt is either bypassed by a flag that becomes boilerplate or hangs a non-interactive caller, and the operation is a near-no-op in normal use — gating the ordinary path trains callers to skip gates. The warning lives in the help text, where an agent reads it.
