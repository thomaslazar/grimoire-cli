...
- **Positional args for value-only subcommands.** Subcommands whose parameters ARE the values, with no ID key, take positional args rather than flags — `config set <key> <value>`. Commands whose body is a handful of scalars take them as flags mirroring the API's body field names — `addons update`, `backups settings set`. Commands carrying a metadata body take it as JSON via `--input`/`--stdin`, validated against the generated model's own field list: `books`, `systems` and `maps` all do, and a hand-written mirror of the API's fields is not to be added.
- **README Commands table and API coverage** are updated in the same PR — see [Docs, specs & roadmap](#docs-specs--roadmap).

`systems list` / `systems get` need no role tag: any authenticated non-guest can read them, so the mechanism is currently exercised only by `RoleSectionTests`. The first write command is what will use it for real.
...
- Expected location: `temp/grimoire/` (gitignored). **Pin it to the release the local stack runs**, never upstream `main`, which carries work no instance runs. That is `v1.7.1` here and the `1.5.6` tag on `support/grimoire-1.5.6`:
  ```bash
  git -C temp/grimoire fetch --depth 1 origin tag v1.7.1
  git -C temp/grimoire checkout v1.7.1
  ```
