...
- **No `--token` flag and no `GRIMOIRE_TOKEN`.** abs-cli has both, and has a referent for them: ABS issues non-expiring user tokens and API keys, so a durable credential can live in the variable. Grimoire has none — `get_current_user` accepts a Bearer header, the session cookie and a deprecated `?token=` query param, all three carrying the same 30-minute JWT that only `login` can mint, and the backend's one `api_key` is `stats_api_key`, a shared secret read from `X-API-Key` for a single stats endpoint. So the flag could only ever hold a 30-minute JWT obtained by logging in, which already writes the config file the flag exists to avoid needing. The token resolves from the file alone.
...
- **`--server` is declared per-subcommand on commands that consume a saved token**, matching abs-cli, and threaded into `CommandHelper.BuildClient` so the flag tier of the server's `flags > env > file` is actually reachable. The token has no flag or environment tier: it comes from the config file alone. Two exceptions, both principled: `login` takes `--server` because it *produces* the token and builds its own client directly; `config` and `self-test` take neither, having no API call at all.
