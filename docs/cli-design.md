...
- **`--help-full`** is a recursive root option that additionally prints two
  ...
  (generated by `tools/GenerateResponseExamples` from the types on
  `AppJsonContext`, checked for drift by `ResponseExamplesDriftTest`) — beside
  a "Request shape" block for write commands, via `AddRequestShape<T>()`,
  sourced from `Commands/RequestExamples.g.cs` (generated by
  `tools/GenerateRequestExamples` from the Kiota models, checked by
  `RequestExamplesDriftTest`). The request sample's field names come from the
