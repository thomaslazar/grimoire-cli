...
- **Binary responses are the one exception to "stdout is JSON."** `systems
  cover get`, `books thumbnail` and `backups download` require
  `--output <path|->`: `-` copies the raw bytes to stdout and prints nothing
  else; a path writes the file and prints a `SavedFile` receipt
  (`{path, bytes}`) instead. `--output -` is the only way bytes reach stdout —
  every other command's stdout is always valid JSON.
