...
Program.cs                 root command, global options, log setup, exit codes
  ...
  Api/                     GrimoireApiClient, ApiEndpoints, QueryBuilder,
                           TokenHelper, DebugHttpHandler
...
2. `CommandHelper.BuildClient()` resolves server and token, exiting 1 if either
   is missing.
3. `SystemsService.ListAsync(...)` composes the query with `QueryBuilder`, which
   omits unset values and URL-encodes the rest — filter values are real system
   names like `Dungeons & Dragons`.
4. `GrimoireApiClient.GetAsync` sends it with the bearer token, warns on stderr
   if the token is near expiry, and on a non-2xx logs a mapped message and exits
   2.
...
Two files are generated and committed, each guarded by a test or a script:
